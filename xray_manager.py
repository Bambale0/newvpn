...
class XrayManager:
    ...
    async def create_client(self, email: str, expiry_date: datetime) -> Optional[Dict]:
        ...
        if result and result.get("success"):
            ...
            return {"id": client_id, "email": email, "config_link": sub_link}
        return None

    async def update_client_expiry(
        self, client_id: str, new_expiry: datetime, enable: bool = True
    ) -> bool:
        """
        ...
        result = await self._api_request(
            "POST", "inbounds/updateClient/" + client_id, update_data
        )
    ...
    async def get_traffic_stats(self, client_id: str) -> Dict:
        ...
        result = await self._api_request(
            "GET", f"inbounds/getClientTraffics/{self.inbound_id}"
        )
        if result and result.get("success"):
